...
## 트리
* 연결 리스트처럼 노드로 이루어진 데이터구조 
* 노드는 자식인 노드만을 가리킬 수있다. 자식이 부모를 가리키는 경우가 있으면 안된다
* 루트: 가장 탑에있는 트리
* 이진트리: 각 노드가 최대 두개의 자식을 가져야한다. 즉 자식이 0,1,2 중 하나
  * 부모 왼쪽에 있는 자식은 부모보다 작고 부모 보다 큰 자식은 부모 오른쪽
  * 추가 logn, 삭제 logn, 찾기 logn      
* 너비우선탐색(BFS) 
* 깊이우선탐색(DFS)
* 트리는 비선형 데이터구조
* 루트하나와 많은 자식 노드들로 구성되어 있다. 

## 힙
* 힙은 트리의 일종, 주로 최댓값 또는 최솟값을 빠르게 찾아내는 우선순위 큐를 구현하는데 사용된다  
* 최소힙: 부모노드의 값이 항상 자식 노드의 값보다 작거나 같은힙
* 최대힙: 부모노드의 값이 항상 자식 노드의 값보다 크거나 같은힙
* 형제들 사이에서는 특별한 순서가 없음
* 추가 logn, 삭제 logn, 찾기 n
* 항상 왼쪽부터 채운다
* 한 노드가 최대 2개의 자식을 가진다

## 해시테이블 = 해시맵
* 해시테이블: 키와 값의 쌍으로 데이터를 저장하는 자료구조
* 자바스크립트, 파이썬, 루비 등등 많은 언어에 기본 유형으로 내장되어있다
* 해시 함수를 사용해서 키의 값을 인덱스로 변환한다 이를 통해 빠른 검색속도를 제공한다
* 빠른 검색속도
  * 검색은 1의 시간 복잡도를 갖는다. 최악의 경우 충돌이 발생할 수 있어서 이에 대한 처리가 필요하다
* 유연한 저장공간
  * 키-값 쌍을 저장하는데 사용되며 크기가 동적으로 조정될 수 있다. 이를 통해 유연하게 데이터를 저장하고 관리할 수 있다.
* 해시함수는 정보보호, 저장, 사이트 로그인 인증등에 도움이 돈다. 암호 기술을 사용하는 암호화폐에서도 사용된다
* 버킷: 테이블에서 데이터를 저장하는 공간, 해시 테이블은 배열과 같은 구조를 사용하여 데이터를 저장하는데 이 배열의 각 요소를 버킷이라고 한다
* 해시함수: 임의의 크기의 데이터를 고정된 크기의 값으로 매핑하는 함수, 입력데이터를 해시값으로 변환하여 해당 값을 사용하여 데이터를 저장하거나 검색하는데 사용
  *  일관성: 동일한 입력에 대해 동일한 출력을 반환해야한다
  *  고유성: 서로다른 입력에 대해 고유한 출력을 반환해야한다. 즉 다른데이터는 가능한 한 다른 해시값으로 매핑되어야한다
  *  고르게분포: 가능한 모든 입력에 대해 해시 값이 고르게 분포되어야한다.
  *  충돌저항성: 두개의 서로다른 입력에 대해 동일한 해시 값이 생성되는 충돌이 최소화 되어야한다.
  * ex)  SHA-1, SHA-256, 
  * 해시 함수는 대부분 소수를 이용한다 
    * 일반적으로 소수는 큰것이 더좋음 -> 충돌이 더 줄어든다
    * 충돌이 줄어든다
  * 데이터가 아주 많은 경우 충돌이 어느정도 일어날 가능성이 높다. 해시 함수가 아주 좋은것일지라도 여전히 충돌은 발생한다.
    * 개별 체이닝: 충돌이 발생한 경우 각 해시 버킷에 연결 리스트와 같은 자료 구조를 사용하여 충돌된 데이터를 저장한다. 이렇게 하면 동일한 해시 값에 여러개의 데이터 저장가능
    * 개방주소법: 충돌이 발생한 경우 해시 테이블 내에서 새로운 위치를 찾아 데이터를 저장하는방법
      * 직선 탐색법: 충돌이 발생한 경우 다음 빈 버킷을 순차적으로 검색하여 데이터를 저장하는 방법
* 추가 1, 삭제1, 접근1

## 그래프
* 유한하교 변할 수 있는 꼭지접이나 노드나 점들의 집합으로 구성된 데이터 구조
* 유한하고 집합에 순서가 없는 경우에는 무방향 그래프, 순서가 있는 경우에는 유방향 그래프
* 정점 = 노드, 간선 = 노드 사이의 연결
* 가중그래프: 간선에 가중치가 할당된 그래프, 두 정점간의 연결 강도를 나타내는값으로 일반적으로 거리, 용량, 비용 등의 의미를 갖는다. 최단 경로 찾기, 네트워크 최대 흐름 등의 문제를 해결하는데 사용된다
* 비가중: 간선에 가중치가 없는 그래프, 모든 간선의 가중치가 동일하다. 가중치가 없는 간단한 형태의 그래프로 연결 여부에만 관심이 있는 경우에 사용된다
* 방향그래프: 간선에 방향이 있는 그래프, 간선은 출발점과 도착점이라는 방향성을 갖는다. 데이터의 흐름이나 의존성을 나타내는데 사용되며 웹페이지의 링크구조, 전자회로의 신호 흐름등의 예시가 있다
* 무방향그래프: 간선에 방향이 없는 그래프, 양방향 연결된것으로 취급한다. 네트워크의 상호 연결성을 나타내는데 사용되며 친구관계, 도로 네트워크 등의 예시가 있다.
* V: 정점의 개수, E: 간선의 개수
* 데이터가 퍼져 있고 간선이 많지 않으면 행렬을 사용하지 않는것이 낫다 -> 노드의 개수가 아주 많고 그게 서로 연결이 되어있지 않은경우가 많은경우 인접리스트를 사용하는게 더 매력적인다
* 인접행렬은 v가 정점의 숫자일때 v제곱의 공간을 차지한다.

## 순회
* 방문, 최신화, 확인, 탐색, 출력 등을 그래프의 모든 정점에 대해 수행하는 것
* DFS순회: 한 정점에서 시작해서 해당 정점과 연결된 모든 정점을 탐색한 후에 그중 하나의 정점을 선택해서 다시 dfs를 수행해서 이과정을 반복하여 그래프의 모든 정점을 방문하는것 -> 재귀함수를 이용해서 구현된다
* BFS순회: 시작정점부터 시작해서 인접한 모든정점을 우선 탐색한 후에 다음 수준의 인접한 정점들을 탐색하는것 -> 큐를 이용해서 구현된다


## 다익스트라 알고리즘
* 그래프의 한 정점에서 모든 다른 정점까지의 최단 경로를 찾는 알고리즘
* 그리디 알고리즘의 일종으로 각 정점까지의 최단 경로를 저장하는 배열을 유지하면서 경유하는 정점을 선택하여 최단 경로를 갱신한다. 
* 네트워크 라우팅 gps 경로탐색, 통신 네트워크 최적화 등에 활용된다
* 우선순위 큐를 사용한다

## 동적 프로그래밍
* 복잡한 문제를 해결하기 위한 알고리즘 설계 기법중 하나, 주어진 문제를 하위 문제로 분할하고 각 하위 문제의 해결 방법을 기록하여 중복 계산을 피하고 최적화 하는 방식으로 동작한다
